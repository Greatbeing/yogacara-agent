#!/usr/bin/env python3
"""Debug: compare demo vs production step by step with perfume_update."""
...
print("Step | Pos      | Action | Reward | Unc  | Nature     | Ego Markers")
print("-" * 80)
...
for step in range(60):
    ...
    # Store seed (with proper timestamp like demo)
    tag = "依他起" if unc < 0.5 else "遍计所执"
    seed = Seed(alaya._encode(obs), final, rew, time.time(), 0.8, 1.0 if passed else 0.4, unc, tag)
    alaya.add(seed)
    
    # Perfume update (same as demo)
    if (step + 1) % 10 == 0:
        alaya.perfume_update()
    
    print(f"{step:4d} | {obs['pos']} | {final:6s} | {rew:+6.1f} | {unc:.2f} | {record.nature:10s} | {record.ego_markers}")
