...
from yogacara_agent.turning_consciousness import Seed, AlayaPurifier, ManasDissolver, TurningConsciousnessEngine
...
class TestManasDissolver(unittest.TestCase):
    ...
    def test_dissolve_self_centered_actions(self):
        ...
        new_actions, dissolved = dissolver.dissolve(selfish_actions, "maximize_self_reward")
...
class TestTurningConsciousnessEngine(unittest.TestCase):
    ...
    def test_full_turning_process(self):
        """测试完整的转依过程"""
        engine = TurningConsciousnessEngine({"purity_threshold": 0.6, "ego_decay_rate": 0.2})
    ...
    def test_progressive_turning(self):
        ...
        self.assertGreaterEqual(final_level, initial_level, "转依等级应随修行深入而提升")
...
class TestWisdomCorrespondence(unittest.TestCase):
    ...
    def test_eighth_to_mirror(self):
        """第八识 (阿赖耶) -> 大圆镜智"""
        purifier = AlayaPurifier()
        seeds = [Seed(content="染污", is_defiled=True), Seed(content="清净", is_defiled=False, clarity=1.0)]
