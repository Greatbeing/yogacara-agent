...
def expose_metrics(): return Response(generate_latest(), media_type=CONTENT_TYPE_LATEST)
def record_step(reward: float, alignment: float, duration: float): STEPS_TOTAL.inc(); REWARD_TOTAL.set(reward); ALIGNMENT_SCORE.set(alignment); STEP_DURATION.observe(duration)
def record_interception(): MANAS_INTERCEPTIONS.inc()
def record_memory_hit(count: int): MEMORY_HITS.inc(count)
