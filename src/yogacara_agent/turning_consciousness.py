...
class TurningConsciousnessEngine:
    ...
    def __init__(self, config: Dict | None = None):
        cfg = config or {}
        self.alaya_purifier = AlayaPurifier(purity_threshold=cfg.get("purity_threshold", 0.7))
        self.manas_dissolver = ManasDissolver(ego_decay_rate=cfg.get("ego_decay_rate", 0.15))
        # 第六识、前五识的逻辑可在此扩展
        self.step_count = 0

    def step(self, seeds: List[Seed], action_probs: Dict[str, float], reward_context: str) -> TurningResult:
        """
        执行一步转依
        """
        self.step_count += 1
        ...
        # --- 2. 转第七识为平等性智 (断除我执) ---
        equalized_probs, dissolved_ego = self.manas_dissolver.dissolve(action_probs, reward_context)
        ...
        # --- 综合计算转依等级 ---
        # 转依不是简单的加法，是根本性质的变化
        turning_level = mirror_level * 0.3 + equality_level * 0.3 + observation_level * 0.2 + action_level * 0.2
...
if __name__ == "__main__":
    ...
    # 充满我执的动作分布
    selfish_actions = {"attack_enemy": 0.8, "hoard_resources": 0.15, "help_others": 0.05}
