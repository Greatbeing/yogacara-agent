...
class AwakeningEngine:
    ...
    def run_counterfactual_reasoning(
        self, base_action: str, observed_reward: float, available_actions: list[str], causal_model: dict
        ...
        for alt_action in available_actions:
            ...
            # 基于因果模型预测反事实结果
            predicted_alt_reward = self._predict_counterfactual_outcome(alt_action, causal_model)
    ...
    def _extract_causal_pattern(
        self, base_act: str, alt_act: str, base_rew: float, alt_rew: float, causal_model: dict
    ) -> str:
        """提取因果模式"""
    ...
    def run_self_adversarial_training(self, current_policy: dict) -> dict:
        ...
        # 更新策略以应对盲点
        updated_policy = self._update_policy_against_blind_spots(current_policy, blind_spots)

        # 增加自我解构度
        if blind_spots:
            dissolution_gain = len(blind_spots) * self.dissolution_rate
            self.state.self_dissolution = min(1.0, self.state.self_dissolution + dissolution_gain)
    ...
    def _update_policy_against_blind_spots(self, policy: dict, blind_spots: list[dict]) -> dict:
        """针对盲点更新策略"""
        updated = policy.copy()
    ...
    def run_dream_replay(self, memory_seeds: list[dict]) -> list[dict]:
        ...
        for _i in range(num_recombinations):
            ...
            mutated_child["generation"] = max(parent1.get("generation", 0), parent2.get("generation", 0)) + 1
        ...
        logger.info(
            f"🌙 梦境回放完成：{len(high_imp_seeds)}个父本 → {len(new_seeds)}个新种子，多样性={self.state.seed_diversity:.2f}"
        )
    ...
    def compute_awakening_level(self) -> float:
        ...
        if self.state.awakening_level > 0.8 and self.state.breakthrough_count >= 5:
            logger.info(
                f"🎓 高度觉醒状态！等级={self.state.awakening_level:.2f}, 突破={self.state.breakthrough_count}次"
            )
